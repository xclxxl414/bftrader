...
void WorkingOrderForm::updateUI()
{
    ...
    for (auto order : orders_) {
        ...
        vItem.insert("direction", ProtoUtils::formatDirection(order.direction()));
        vItem.insert("offset", ProtoUtils::formatOffset(order.offset()));
        vItem.insert("price", order.price());
        vItem.insert("totalVolume", order.totalvolume());
        vItem.insert("tradedVolume", order.tradedvolume());
        vItem.insert("status", ProtoUtils::formatStatus(order.status()));
